...
# --- 4. MOTORES DE CÁLCULO (FX FIX CERTIFICADO) ---
def clean_df(df):
...
def get_fx_rate():
    try:
        # Uso de history para asegurar el spot price real de ~17.52
...
# --- 11. AGENTE TOBIAS (RESTAURACIÓN FIX VISIBILIDAD) ---
resumen_tobias = []
if portfolio:
    for t, info in portfolio.items():
        if t in active_data:
            ...
            resumen_tobias.append(f"{t}: {int(info['shares'])} títulos, retorno {pnl_p:+.2f}%")
    portfolio_context = " | ".join(resumen_tobias)
else:
    portfolio_context = "Sin posiciones"

# Inyectamos el CSS directamente en el componente para forzar la visibilidad
tobias_html = f"""
<div style="position: fixed; bottom: 20px; right: 20px; z-index: 999999;">
    <elevenlabs-convai 
        agent-id="agent_4901kqp1gs5bfqstk9zw2p61rpe8"
        dynamic-variables='{{"portfolio_context": "{portfolio_context}"}}'
