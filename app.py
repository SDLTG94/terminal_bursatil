...
# --- 11. AGENTE TOBIAS (NUEVO FORMATO DE DATOS) ---
# Generamos un resumen amigable pero estructurado para que Tobias lo lea sin fallas
resumen_tobias = []
if portfolio:
    for t, info in portfolio.items():
        if t in active_data:
            ...
            resumen_tobias.append(
                f"Activo: {t}, Títulos: {int(info['shares'])}, Costo Promedio: ${info['total_net_cost']/info['shares']:.2f} MXN, "
                f"Precio Actual: ${m['p_usd']:.2f} USD, Rendimiento Actual: {pnl_pct:+.2f}%"
            )
    portfolio_context = " | ".join(resumen_tobias)
else:
    portfolio_context = "El usuario no tiene posiciones abiertas actualmente."

# Serializamos las variables dinámicas como un JSON limpio
