...
st.markdown("""
    ...
    /* Ajuste para eliminar espacio muerto en el fondo */
    .stApp { bottom: 0px; }
    </style>
...
# --- 4. MOTORES DE CÁLCULO (FX FIX ROBUSTO PROYECTO 1.1) ---
def clean_df(df):
...
def get_fx_rate():
    try:
        # Lógica de Ticker.history para evitar estancamiento en 18.50
...
if portfolio:
    for t, info in portfolio.items():
        if t in active_data:
            ...
            pnl_pct = (net_pnl / info["total_net_cost"]) * 100
...
if df_t is not None:
    ...
    # Restauración de indicadores técnicos de Proyecto 1.1
...
# --- 11. AGENTE TOBIAS (FIX ESPACIO MUERTO) ---
sum_list = []
for t, info in portfolio.items():
    if t in active_data:
        m = active_data[t]
...
# Fix Espacio Muerto: div con position fixed y height=0 en components.html
tobias_html = f"""
<div style="position: fixed; bottom: 20px; right: 20px; z-index: 9999; overflow: visible;">
    <elevenlabs-convai 
...
"""
# Al usar height=0 eliminamos el bloque blanco al final del dashboard
components.html(tobias_html, height=0)

# --- 12. LATIDO DE ACTUALIZACIÓN (AUTO-REFRESH 60S) ---
